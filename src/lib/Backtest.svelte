...
<div class="glass rounded-xl p-6 lg:p-8">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2">
      ...
      {:else}
        <div class="h-full min-h-[400px] flex flex-col items-center justify-center bg-surface-container-low rounded-xl border border-white/5 border-dashed text-on-surface-variant">
          <span class="material-symbols-outlined text-[64px] mb-4 opacity-50">monitoring</span>
          <h3 class="text-title-md font-bold text-on-surface">Area Hasil Backtest</h3>
          <p class="text-sm max-w-md text-center mt-2">Pilih konfigurasi Anda di panel sebelah kiri dan jalankan simulasi untuk melihat grafik keuntungan, win rate, dan riwayat pesanan (order history).</p>
        </div>
      {/if}
    </div>
  </div>
